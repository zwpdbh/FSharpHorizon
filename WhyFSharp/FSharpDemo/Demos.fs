...
module Demo01 =
  ...
  let demo02 () = compress [ 1; 1; 2; 2; 3; 3; 3; 3; 4 ]
...
module Demo04 =
  ...
  let gameStatus (game: Game) =
    ...
    | InProcess (player01, player02) -> $"Game is on: {player01} vs {player02}"
    | Finished player -> $"Game is finished: {player} is the winner!"

  let demo () =
    let game01 = NotStarted

    let game02 =
      InProcess({ name = "player01"; score = 0 }, { name = "player02"; score = 0 })
...
module Demo06 =
  ...
  let extractHost str =
    match str with
    | FirstRegexGroup "http://(.*?)/(.*)" host -> host
    | FirstRegexGroup ".*?@(.*)" host -> host
    | FirstRegexGroup "It will be None, therefore not matched" host -> host
    | _ -> str

  let demo () =
    extractHost "http://google.com/test"
    |> printfn "%A"
...
module Demo07 =
  ...
  let loadCustomer customerId =
    asyncResult {
      do!
        customerId = 42
        |> Result.requireTrue "customer not found"
  ...
  let loadData dataId =
    asyncResult { return { Id = dataId; Amount = 100 } }
  ...
  let demo () = compute () |> Async.RunSynchronously
...
module Demo08 =
  ...
  let demo () =
    [ 5..20 ]
    |> List.iter (fun n -> measureNworkers n 30 40)
...
module DemoAsycSeq =
  ...
  let useAsyncSeqV5 (input: int list) =
    ...
    |> AsyncSeq.ofSeq
    |> AsyncSeq.mapAsync someCompute
    |> AsyncSeq.chooseAsync (fun x -> async { return id x })
